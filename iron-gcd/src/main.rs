...
fn post_gcd(request: &mut Request) -> IronResult<Response> {
    ...
    // Get unparsed numbers from form data
    let unparsed_numbers = match form_data.get("n") {
        None => {
            response.set_mut(status::BadRequest);
            response.set_mut(format!(
                "Form contains no numbers. Please fill all the required data. :)"
            ));
            return Ok(response);
        }
        Some(nums) => nums,
    ...
    for unparsed in unparsed_numbers {
        match u64::from_str(&unparsed) {
            Err(_) => {
                response.set_mut(status::BadRequest);
                response.set_mut(format!("{:?} is not a number", unparsed));
                return Ok(response);
            }
            Ok(n) => numbers.push(n),
        }
